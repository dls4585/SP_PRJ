    ...
    while(1) { // 아직 중간 파일에 안 썼음
        ...
        *lines+=5;
        ...
        if(asm_tokens == 3) { // label 있음
            if(opcode_search(optab, line_token[0]) == -1) { // symbol 이라는 말
            ...
            strcpy(opcode, line_token[0]);
            strcpy(operand, line_token[1]);
            fprintf(mid_file, "%d\t%X\t-\t%s\t%s\n", *lines, *LOCCTR, opcode, operand);
        }
        else {
            strcpy(opcode, line_token[0]);
            fprintf(mid_file, "%d\t%X\t-\t%s\t-\n", *lines, *LOCCTR, opcode);
        }
        ...
        if (strcmp(opcode, "END") == 0) break;
            ...
            *LOCCTR += 4;
            ...
            if(opcode_search(optab, opcode) != -1 || strcmp(opcode, "WORD") == 0) {
                *LOCCTR += 3;
            }
