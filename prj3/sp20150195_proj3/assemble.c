        ...
        *lines+=5;
        ...
        if(line[0] == '.') {
            ...
            if(asm_tokens == 3) {
                ...
                if(search_opcode(optab, line_token[0]) == NULL) {
                    ...
                    if(strcmp(mnemonic, "START") != 0 && strcmp(mnemonic, "END") != 0) {
                        ...
                        if(search_symbol(symtab, symbolNode->name) != FAIL) {
                            free(symbolNode);
                            *error_flag = 2;
                            return FAIL;
                        }
                        // 새로운 symbol을 symbol table에 삽입한다.
                        insert_symbol(symtab, symbolNode);
                    }
                }
                else {
            ...
            if(strcmp(mnemonic, "END") == 0); // END의 경우 아무런 처리를 하지 않는다.
                ...
                if((opcode_node = search_opcode(optab, mnemonic)) != NULL) {
...
int pass2(char* filename, bucket* optab, bucket* symtab, line_list* linelist, int* prgm_len) {
    ...
    while(current != NULL) {
        ...
        if(strcmp(mnemonic, "LDB") == 0) {
            base = search_symbol(symtab, operand);
        }
        ...
        if(strcmp(mnemonic, "START") == 0) {
        ...
        else if((opcode_node = search_opcode(optab, mnemonic)) == NULL) {
            ...
            if((target_addr = search_symbol(symtab, operand)) == FAIL) { // operand가 symbol이 아니면
                target_addr = (int)strtol(operand, NULL, 10);
